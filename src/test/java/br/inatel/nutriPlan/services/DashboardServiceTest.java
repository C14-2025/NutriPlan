...
public class DashboardServiceTest {
    ...
    void testCalcularMacrosPorDia_SomaCorreta() {
        ...
        when(refeicaoService.findByUsuarioId(userId))
                .thenReturn(Arrays.asList(r1, r2));

        when(refeicaoService.calcularTotaisNutricionais(1L))
                .thenReturn(Map.of("Calorias", 500.0, "Carboidratos", 60.0, "Proteinas", 30.0, "Gorduras", 20.0));

        when(refeicaoService.calcularTotaisNutricionais(2L))
                .thenReturn(Map.of("Calorias", 700.0, "Carboidratos", 90.0, "Proteinas", 40.0, "Gorduras", 30.0));
    ...
    void testCalcularDistribuicaoCaloricaPorDia() {
        ...
        when(refeicaoService.calcularTotaisNutricionais(1L)).thenReturn(Map.of("Calorias", 800.0, "Carboidratos", 100.0, "Proteinas", 50.0, "Gorduras", 30.0));
    ...
    void testGerarRelatorioSemanal() {
        ...
        when(refeicaoService.calcularTotaisNutricionais(1L)).thenReturn(Map.of("Calorias", 400.0, "Carboidratos", 80.0, "Proteinas", 40.0, "Gorduras", 15.0));
        when(refeicaoService.calcularTotaisNutricionais(2L)).thenReturn(Map.of("Calorias", 600.0, "Carboidratos", 120.0, "Proteinas", 60.0, "Gorduras", 25.0));

        List<Map<String, Object>> result = dashboardService.gerarRelatorioSemanal(userId);

        assertEquals(7, result.size());

        Map<String, Object> resDia1 = result.stream().filter(r -> r.get("day").equals(dia1.toString())).findFirst().orElse(null);
        Map<String, Object> resDia2 = result.stream().filter(r -> r.get("day").equals(dia2.toString())).findFirst().orElse(null);
